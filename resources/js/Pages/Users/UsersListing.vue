...
<AuthenticatedLayout>
    <template #header>      
    <Link :href="route('add')" class="text-black bg-green-700 hover:bg-green-800 focus:outline-none focus:ring-4 focus:ring-green-300 font-medium rounded-full text-sm px-5 py-2.5 text-center mr-2 mb-2 dark:bg-green-600 dark:hover:bg-green-700 dark:focus:ring-green-800">
        Add
    </Link>                       
    </template>  
          ...
          <div class="bg-white rounded-md shadow overflow-x-auto">
            <table class="w-full whitespace-nowrap">
              <tr class="text-left font-bold">
                ...
                <tr class="hover:bg-gray-100 focus-within:bg-gray-100" v-for="(user, index)  in users" :key="user.id">
                  <td class="border-t m-2 text-center">{{ index + 1 }}</td>
                  <td class="border-t m-2 text-left">{{ user.name }}</td>
                  <td class="border-t m-2 text-left">{{ user.description }}</td>                  
                  <td class="w-px border-t m-2 text-center">
                    <div class="m-4">                                                  
                      <Link :href="route('blog.edit', user.id)" class="text-white bg-green-700 hover:bg-green-800 focus:outline-none focus:ring-4 focus:ring-green-300 font-medium rounded-full text-sm px-5 py-2.5 text-center mr-2 mb-2 dark:bg-green-600 dark:hover:bg-green-700 dark:focus:ring-green-800">
                                  Edit
                      </Link>                                                              
                    </div>                  
                    <div class="m-4">                                                  
                      <button @click="confirmDelete" :data-id=user.id class="text-white bg-red-700 hover:bg-red-800 focus:outline-none focus:ring-4 focus:ring-green-300 font-medium rounded-full text-sm px-5 py-2.5 text-center mr-2 mb-2 dark:bg-green-600 dark:hover:bg-green-700 dark:focus:ring-green-800">
                                  Delete
                      </button>                                                              
                    </div>                    
                  </td>
